...
fn main() {
    ...
    let files: Vec<_> = WalkDir::new(root_dir).into_iter().filter_map(|e| e.ok())
        .filter(|e| e.file_type().is_file() && e.file_name().to_string_lossy().ends_with(".pdf")).collect();
    ...
    for entry in files {
        ...
        for i in 0..rendered_pages[0].len() {
            ...
            let path = {
                ...
                pb.line_to(max_width as f32 + stroke_width, max_height as f32 + stroke_width);
            ...
            let mut pixmap = Pixmap::new(((max_width as f32 + stroke_width * 2.0) * 5.0) as u32, (max_height as f32 + stroke_width * 2.0) as u32).unwrap();
            ...
            for j in 0..5 {
                ...
                pixmap.draw_pixmap(0, 0, Pixmap::decode_png(&rendered_pages[j][i]).unwrap().as_ref(),
                                   &PixmapPaint::default(), Transform::from_translate(cursor.tx + stroke_width / 2.0, cursor.ty + stroke_width /  2.0), None);
